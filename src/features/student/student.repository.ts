...
export class StudentRepository {
  ...
  async create(data: CreateStudentDto) {
    const id = randomUUID();
    const { email, password, firstName, lastName, userCode, phone, avatar, parentId } = data;
    const resolvedEmail = email ?? `${userCode || id}@student.local`;
    const resolvedUsername = resolvedEmail.split('@')[0];
  ...
  async findAll(query: GetStudentsQueryDto & { classIdFilter?: string }) {
    ...
    if (studentIds.length > 0) {
      if (effectiveClassId) {
        ...
        return {
          data: filtered.map((r) => this.mapRow(r, classMap)),
    ...
    return {
      data: rows.map((r) => this.mapRow(r, classMap)),
