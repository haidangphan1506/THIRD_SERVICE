...
type UserRow = { id: string; firstName: string; lastName: string; avatar: string | null; role: string | null };
...
export class ConversationService {
  ...
  async createDirectConversation(userId: string, targetUserId: string) {
    ...
    await this.validateChatPermission(currentUser[0].role ?? 'STUDENT', targetUser[0].role ?? 'STUDENT', userId, targetUserId);
  ...
  async createGroupConversation(userId: string, name: string, participantIds: string[]) {
    ...
    await this.db.insert(conversationParticipants).values(
      allIds.map((id) => ({ conversationId: newConv.id, userId: id })),
    );
  ...
  async createClassConversation(userId: string, classId: string) {
    ...
    await this.db.insert(conversationParticipants).values(
      participantIds.map((id) => ({ conversationId: newConv.id, userId: id })),
    );
  ...
  private async validateChatPermission(
    ...
    if (currentUserRole === 'TUTOR') {
      ...
      if (targetUserRole === 'STUDENT' || targetUserRole === 'PARENT') {
        ...
        if (!hasAccess) {
          throw new ForbiddenException('Tutor can only chat with students/parents in their classes');
        }
        return;
      }
    }
  ...
  private async checkTutorStudentAccess(tutorId: string, studentOrParentId: string) {
    ...
    const enrollment = await this.db
      ...
      .where(
        and(
          sql`${classStudents.classId} IN ${classIds}`,
    ...
    switch (userRole) {
      case 'ADMIN':
        chatableUsers = await this.db
          .select({ id: users.id, firstName: users.firstName, lastName: users.lastName, avatar: users.avatar, role: users.role })
          .from(users)
