  ...
  getToolDeclarations(): AiToolDeclaration[] {
    ...
    return [
      ...
      },
      {
        ...
        description:
          'Các buổi học cụ thể theo ngày giờ trong một khoảng thời gian (mặc định từ hiện tại đến 7 ngày tới). Dùng khi hỏi "hôm nay/ngày mai/tuần này có buổi học nào".',
        parametersJsonSchema: {
          type: 'object',
          properties: {
            fromIso: { type: 'string', description: 'Mốc bắt đầu dạng ISO 8601 (tùy chọn).' },
            toIso: { type: 'string', description: 'Mốc kết thúc dạng ISO 8601 (tùy chọn).' },
          },
        },
      },
      {
      ...
      },
      {
        ...
        description:
          'Bài tập trong các lớp của người dùng, kèm trạng thái và điểm. Dùng khi hỏi về bài tập, deadline, bài chưa nộp.',
        parametersJsonSchema: {
          type: 'object',
          properties: {
            status: {
